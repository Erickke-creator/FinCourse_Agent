...
class MLPredictor:
    ...
    def load_models(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(risk_path):
                risk_path = rating_path  # use rating for risk as fallback
            ...
            if os.path.exists(risk_path) and risk_path != rating_path:
                self.rf_risk = joblib.load(risk_path)
            else:
                self.rf_risk = self.gb_rating  # fallback
            ...
            print(f"[ML] Models loaded: default={type(self.xgb_default).__name__}, "
                  ...
                  f"risk={type(self.rf_risk).__name__}")
            return True
    ...
    def _build_feature_vector(self, enterprise_data: Dict[str, Any]) -> np.ndarray:
        ...
        # Tax score: sourced from KB tax_level_scoring.csv or legacy fallback
        tax_score_map = {'A': 5, 'B': 4, 'M': 3, 'C': 2, 'D': 1}  # legacy fallback
