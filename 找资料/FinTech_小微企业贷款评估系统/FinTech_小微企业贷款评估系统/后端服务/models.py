...
class KBSourceEntry(BaseModel):
    """Traceability record for a single knowledge base lookup."""
...
class MLPredictionResult(BaseModel):
    ...
    risk_level_ml: Optional[str] = None
...
class EvaluationResult(BaseModel):
    ...
    kb_sources: List[KBSourceEntry] = []    # Knowledge base traceability
    kb_version: str = ""                     # KB version stamp
...
class KBMetadataResponse(BaseModel):
    """Response for /api/kb/metadata endpoint."""
    version: str
    file_count: int
    data_stats: dict = {}
...
class KBSourceListResponse(BaseModel):
    """Response for listing KB sources used in last evaluation."""
    sources: List[KBSourceEntry] = []
    kb_version: str = ""
