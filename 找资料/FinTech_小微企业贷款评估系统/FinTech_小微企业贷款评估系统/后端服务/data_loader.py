...
def generate_enhanced_synthetic_data(n: int = 8000) -> pd.DataFrame:
    """
    ...
    # Need larger offset since signals are stronger
    threshold_shift = np.percentile(default_score, 88)  # 88th percentile → ~12% above
...
def get_training_data(n_synthetic: int = 8000) -> pd.DataFrame:
    ...
    Returns real CUMCM data if available, otherwise enhanced synthetic data.
    """
    # Try real data first
