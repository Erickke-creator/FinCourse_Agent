...
Knowledge-base-aware: accepts optional kb_query parameter for all data lookups.
When kb_query is provided, data is sourced from the unified knowledge base
with full traceability. When absent, falls back to legacy hardcoded data.

See kb/loader/querier.py for the KBQuery interface.
"""
...
# Extend sys.path to find kb/loader
_KB_LOADER_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "kb", "loader"
)
_KB_LOADER_PATH = os.path.abspath(_KB_LOADER_PATH)
...
# ================================================================
# Legacy hardcoded data — used as fallback when kb_query is absent
# ================================================================

TAX_LEVEL_SCORE: Dict[TaxLevel, float] = {
    TaxLevel.A: 5.0, TaxLevel.B: 4.0, TaxLevel.M: 3.0, TaxLevel.C: 2.0, TaxLevel.D: 1.0,
}
...
# ================================================================
# KB-aware data access helpers
# ================================================================

def _get_tax_score(tax_level: TaxLevel, kb_query: Optional[Any] = None) -> float:
    """Get tax score from KB if available, else fallback to hardcoded."""
    if kb_query is not None:
        try:
            return kb_query.get_tax_score(tax_level.value)
        except Exception:
            pass
    return TAX_LEVEL_SCORE.get(tax_level, 3.0)


def _get_industry_factor(industry: IndustryType, region: str,
                         kb_query: Optional[Any] = None) -> float:
    """Get industry acceptance factor, with optional regional adjustment."""
    factor = INDUSTRY_ACCEPTANCE.get(industry, 0.70)
    if kb_query is not None:
        try:
            info = kb_query.get_industry_acceptance(industry.value)
            factor = float(info.get("接受度系数", factor))
            # Apply regional adjustment
            adj = kb_query.get_regional_adjustment(industry.value, region)
            factor *= adj
        except Exception:
            pass
    return factor
...
def _get_banks(kb_query: Optional[Any] = None) -> List[Dict]:
    """Get bank list from KB if available, else fallback to hardcoded BANKS_DB."""
    if kb_query is not None:
        try:
            kb_banks = kb_query.get_all_banks()
            if kb_banks:
                return kb_banks
        except Exception:
            pass
    return BANKS_DB  # legacy fallback
...
def score_enterprise_health(inp: LoanInput,
                           kb_query: Optional[Any] = None) -> Tuple[float, List[str], List[str]]:
    """Calculate enterprise health score (0-100). Uses KB for tax scoring when available."""
    risks: List[str] = []
    reasons: List[str] = []
    total = 0.0
    ...
    # 4. Tax compliance (15 pts) — sourced from KB or fallback
    tax_val = _get_tax_score(inp.tax_level, kb_query)
    tax_score = tax_val * 2.0
    if tax_val >= 4:
...
def calculate_bank_approval(inp: LoanInput, bank: dict, health_score: float,
                            kb_query: Optional[Any] = None) -> BankMatchResult:
    """Calculate approval probability for a specific bank. KB-aware for industry/rejection data."""
    base_prob = bank.get("estimated_base_approval", bank.get("estimated_approval_rate", 0.5))

    # Business age check — use KB rejection_sensitivity if available
    if inp.operating_years < bank.get("min_business_years", 1):
        if kb_query and "rejection_sensitivity" in bank:
            sens = bank["rejection_sensitivity"].get("short_business_age", "medium")
            age_penalty = {"high": 0.3, "medium": 0.5, "low": 0.7}.get(sens, 0.5)
            base_prob *= age_penalty
        else:
            base_prob *= 0.5

    # Collateral requirement
    req_coll = bank.get("requires_collateral_strict", False)
    if not req_coll and "rejection_sensitivity" in bank:
        req_coll = bank["rejection_sensitivity"].get("no_collateral") == "high"
    if req_coll and not inp.has_real_estate:
        if kb_query and "rejection_sensitivity" in bank:
            sens = bank["rejection_sensitivity"].get("no_collateral", "medium")
            coll_penalty = {"high": 0.2, "medium": 0.3, "low": 0.5}.get(sens, 0.3)
            base_prob *= coll_penalty
        else:
            base_prob *= 0.3

    # Credit impact — use KB rejection_sensitivity
    if inp.has_overdue_record:
        if kb_query and "rejection_sensitivity" in bank:
            sens = bank["rejection_sensitivity"].get("credit_issues", "medium")
            credit_penalty = {"high": 0.25, "medium": 0.4, "low": 0.6}.get(sens, 0.4)
            base_prob *= credit_penalty
        else:
            base_prob *= 0.4
    ...
    # Industry acceptance — sourced from KB or fallback
    industry_factor = _get_industry_factor(inp.industry, inp.region, kb_query)
    ...
    tax_ok = _get_tax_score(inp.tax_level, kb_query) / 5.0
...
def evaluate_loan(inp: LoanInput, kb_query: Optional[Any] = None) -> EvaluationResult:
    """Main evaluation function — KB-aware pipeline with traceability support."""
    # 1. Cash flow
    ...
    # 6. Bank matching — sourced from KB when available
    health_score, health_risks, health_reasons = score_enterprise_health(inp, kb_query)
    banks_to_evaluate = _get_banks(kb_query)
    bank_matches = [
        calculate_bank_approval(inp, bank, health_score, kb_query)
        for bank in banks_to_evaluate
    ]
